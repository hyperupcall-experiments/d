...
int main(int argc, char *argv[]) {
   ...
   if (command == CommandPrint) printf("[\n");
   for (int i = 0; ; i += 1) {
		...
		for (int j = 0; ; j += 1) {
			...
			if (command == CommandPrint) {
				printf("\t{ \"category\": \"%s\", \"source\": \"%s\", \"destination\": \"%s\" }", entry.category, entry.source, entry.destination);
	...
	// Check trailing slash.
	{
		if (destination_path[strlen(destination_path) - 1] == '/') {
			if (source_path[strlen(source_path) - 1] != '/') {
				fprintf(stderr, "Error: If destination path does have trailing slash, then source path must have it too.\n");
				exit(1);
			}
